...
(()=>{
 ...
 const model=(id,name)=>({id,display_name:name,context:{nominal:null,effective:null,source:'UNKNOWN'},tokenizer:{mode:'estimated',name:null},vision:null,tools:'emulated',reasoning:{supported:null,control_observed:true},confidence:'OBSERVED'});
 const provider=(id,label,origin,state,models=[])=>({id,label,origin,url:origin+'/',current_url:origin+'/',state,open_tab:true,pinned:false,active:false,last_seen:now,mapping_version:state==='READY'?12:0,last_error:'',context_hint:0,reasoning_value:'',models,mappings:{prompt:8,send:12,response:5,stop:13,new_chat:2,model:3,reasoning:10,attachment:9}});
 const snapshot={protocol:1,backend:'zag',version:'0.1.0',api:{host:'127.0.0.1',port:7331,desired_port:7331,running:true,key_present:true},providers:[
 ...
 async function request(op,params){calls.push({op,params:clone(params)});let result=null;
  ...
  case'provider.update':{const p=get(params.provider_id);for(const k of['label','pinned','context_hint','reasoning_value'])if(k in params)p[k]=params[k];break;}
 ...
 const runtime={transformCallback(fn){const id=next++;callbacks.set(id,fn);return id;},unregisterCallback(id){callbacks.delete(id);},async invoke(command,args={}){
  ...
  switch(command){case'plugin:event|listen':{const id=next++;listeners.set(id,args);return id;}case'plugin:event|unlisten':listeners.delete(args.eventId);return;
   ...
   case'document_import':return{schema:1,origin:'http://127.0.0.1:7340',recipes:[],manual_model:''};
