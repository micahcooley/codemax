...
## dev.7 audit corrections

The library pins published package releases rather than assuming the versions in repository package manifests were published. These are direct-version pins, not a fully locked transitive dependency tree or a security certification. Runtime/package installation happens only after the user authorizes the process. Package-manager hooks and dependencies run with that process's user privileges; review the definition before connecting.

The file and Git library recipes require a specific absolute folder and reject filesystem root, relative paths, parent traversal and control bytes. The path is passed as a distinct argv element. The native directory picker is declared in both the main-only command permission and AppManifest; it neither reads files nor starts a process. The server implements its access policy. A starting Git directory is not a guaranteed confinement boundary. The Fetch server can access local/internal addresses: its UI warning and call arguments must be reviewed rather than assuming a public-network sandbox.
