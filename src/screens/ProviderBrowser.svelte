...
<section class="browser-layout" aria-label="Website browser">
 ...
 {#if app.provider}<BrowserPane provider={app.provider}/>{:else}
  <div class="empty-website"><div class="new-site-content"><div class="new-site-mark"><Icon name="globe" size={26}/><span class="label">Your browser. Your models.</span></div><h1>Open a website.<br/>Connect what’s already yours.</h1><p>Use your AI websites in a dedicated browser, then make their models available to your local coding tools.</p><form class="new-site-form" onsubmit={event=>{event.preventDefault();void app.addWebsite(url);}}><Icon name="search" size={18}/><input aria-label="Website to open" bind:value={url} placeholder="Enter an AI chat website" autocomplete="url" spellcheck="false" maxlength="2048" required/><button class="primary" disabled={!app.ready||!url.trim()} aria-label="Open website"><Icon name="arrow" size={17}/></button></form><div class="site-shortcuts">{#each shortcuts as site}<button disabled={!app.ready} onclick={()=>app.addWebsite(site.url,site.name)}><span class="workspace-logo">{site.mark}</span><span><strong>{site.name}</strong><small>Open website</small></span></button>{/each}</div><div class="start-footer"><Icon name="shield" size={16}/><span>Sign in on the website itself. Passwords, cookies, and authentication tokens stay in the provider’s isolated browser profile.</span></div></div></div>
 {/if}</div>
