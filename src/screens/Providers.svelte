<script lang="ts">
 import {app} from '../lib/state/app.svelte';import Icon from '../lib/components/Icon.svelte';import {number,providerText,initials,modelStatus,modelReady} from '../lib/format';
...
<section class="screen"><div class="screen-inner">
 ...
 <table class="permission-table"><tbody><tr><td>Current model</td><td>{current.current_model||'Not exposed yet'}</td></tr><tr><td>Reasoning levels</td><td>{current.reasoning_modes?.length?current.reasoning_modes.map(m=>m.label).join(' · '):current.mappings.reasoning?'Control observed; options unknown':'Unknown'}</td></tr>{#each current.models as model(model.id)}<tr><td>{model.display_name} · context</td><td>{current.context_hint?`${number(current.context_hint)} · user supplied`:model.context.nominal?`${number(model.context.nominal)} · website reported`:'Unknown'}</td></tr><tr><td>{model.display_name} · tokenizer</td><td>{model.tokenizer.name?`${model.tokenizer.name} · identity only`:'Unknown; usage remains estimated'}</td></tr>{/each}</tbody></table>
