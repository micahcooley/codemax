<script lang="ts">
 import {app} from '../state/app.svelte';import Icon from '../components/Icon.svelte';

 let selected=$state('');
 const model=$derived(app.exposedModels.find(m=>m.id===selected)||app.exposedModels.find(m=>m.display_name.toLowerCase().replaceAll(' ','-')==='glm-5.3-flash')||app.exposedModels.find(m=>m.id===app.preferences.default_model)||app.exposedModels[0]);
 ...
 const boundModel=$derived(app.exposedModels.find(m=>m.id===probe?.model));
...
<div class="file-probe-panel"><div>
 <header class="screen-head"><div><div class="breadcrumb">Workspace / Website clients</div><h2>File permission diagnostic</h2><p>Use your website session and its quota. No provider API key. No paid API connection.</p></div><button class="secondary" disabled={!app.ready} onclick={()=>app.addWebsite('https://chat.z.ai/','Z.ai')}><Icon name="globe" size={15}/>Open Z.ai</button></header>
 <div class="split"><div>
  ...
  <section class="section"><label class="field"><span>Website model</span><select aria-label="Website test model" bind:value={selected} disabled={!app.exposedModels.length||busy||needsPermission}><option value="">{model?`${model.provider.label} / ${model.display_name}`:'Open a website to discover models'}</option>{#each app.exposedModels as m(m.id)}<option value={m.id}>{m.provider.label} / {m.display_name}</option>{/each}</select></label>
  ...
  <section class="section"><h3>Two different connections</h3><p style="margin-top:12px"><strong>Website:</strong> your normal chat session and website quota.</p><p style="margin-top:12px"><strong>Coding client:</strong> a local tool talking to Bridge. The loopback address and token in Connect a client protect that local connection, not a provider API.</p></section>
 </aside></div>
