<script lang="ts">
 ...
 const label=$derived(!online?'Offline':warning?(provider.state==='LOGIN_REQUIRED'?'Sign-in required':provider.state==='RATE_LIMITED'?'Website limit reached':'Connection needs attention'):provider.active||provider.browser_busy?'Generating':loading||provider.state==='LOADING'?'Loading':working?'Discovering controls':provider.state==='DISCOVERING'?'Needs control mapping':provider.open_tab&&provider.state==='READY'?'Idle':'Sleeping');
</script>
