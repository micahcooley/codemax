<script lang="ts">
 ...
 let selection=$state(''),folder=$state(''),error=$state(''),picking=$state(false),write=$state(false),commands=$state(false);
 ...
 function use(){try{onchoose(recipeDefinition(selection,folder,{write,commands}));error='';selection='';folder='';}catch(e){error=String(e instanceof Error?e.message:e);}}
</script>
<section class="tool-library" aria-label="Tool library">
 <p class="field-hint">Add only what you need. Local project tools are bundled. Other entries are optional external servers. Nothing runs until you approve its local process.</p>
 <div class="recipe-list">{#each toolRecipes as recipe(recipe.id)}
  <button class="recipe-row" class:selected={selection===recipe.id} aria-pressed={selection===recipe.id} disabled={disabled||picking} onclick={()=>{selection=recipe.id;folder='';error='';write=false;commands=false;}}>
 ...
 {#if selected}<div class="recipe-review" aria-label={`Configure ${selected.label}`}>
  ...
  {#if selected.bundled}
   <label class="setting-line"><div><strong>Allow project changes</strong><p>Enable file writes, edits, moves and recoverable deletion. Each tool call still needs your approval.</p></div><input type="checkbox" aria-label="Enable project changes" bind:checked={write}/></label>
   <details class="advanced-client"><summary>Advanced · trusted command execution</summary><label class="setting-line"><div><strong>Enable shell commands</strong><p>Commands run as your user, not in an operating-system sandbox. They can read or change files outside this project and access the network. Only enable this for trusted tasks.</p></div><input type="checkbox" aria-label="Enable trusted commands" bind:checked={commands}/></label></details>
  {/if}
