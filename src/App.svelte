...
<div class="app-shell" class:compact={app.preferences.compact} style={`--inspector:${app.inspectorWidth}px`}>
 ...
 <div class="toolbar"><div class="navigation-buttons"><button class="icon-button" aria-label="Back" disabled={app.route==='browser'?!app.provider:!app.canGoBack} onclick={()=>app.back()}><Icon name="back" size={16}/></button><button class="icon-button" aria-label="Forward" disabled={!app.canGoForward&&(app.route!=='browser'||!app.provider)} onclick={()=>app.forward()}><Icon name="arrow" size={16}/></button><button class="icon-button" aria-label="Reload website" disabled={!app.provider||app.route!=='browser'} onclick={()=>app.control('reload')}><Icon name="refresh" size={15}/></button></div>
  ...
  <nav class="chrome-shortcuts" aria-label="Codemax tools">
   ...
   <button class:active={app.route==='tools'} aria-current={app.route==='tools'?'page':undefined} aria-label="Tools & MCP" title="Tools and permissions" onclick={()=>app.navigate('tools')}><Icon name="terminal" size={15}/><span>Tools</span>{#if app.activeTask?.state==='PERMISSION_REQUIRED'}<span class="attention-count" aria-label="1 tool approval waiting">1</span>{/if}</button>
 ...
 <div class="workspace">
  <main class="workspace-content">
   ...
   {#if app.activeTask&&app.route!=='tools'}<div class="task-status" class:attention={['PERMISSION_REQUIRED','RESULT_READY'].includes(app.activeTask.state)} role="status"><Icon name="shield" size={15}/><span>{app.activeTask.state==='PERMISSION_REQUIRED'?'A website task needs your approval. No tool will run until you review it.':app.activeTask.state==='RESULT_READY'?'Tool result ready. Continue the conversation when you are ready.':'Website tool task running. You can keep browsing.'}</span><button class="secondary" onclick={()=>app.navigate('tools')}>{app.activeTask.state==='PERMISSION_REQUIRED'?'Review tool call':'View task'}</button></div>{/if}
