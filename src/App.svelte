<script lang="ts">
 ...
 import {routes,type Route} from './lib/types/bridge';import {initials,errorText,recoveryAction} from './lib/format';
...
{:else if app.popup==='shortcuts'}<Dialog title="Keyboard shortcuts">{#each [['Address bar','Ctrl L'],['Provider sidebar','Ctrl Shift B'],['Commands','Ctrl K'],['New website tab','Ctrl T'],['Close current tab','Ctrl W'],['Next / previous tab','Ctrl Tab / Ctrl Shift Tab'],['Switch website tabs','Ctrl 1–9'],['Find on website','Ctrl F'],['Back / forward','Alt ← / →'],['Zoom in / out','Ctrl + / −'],['Reset zoom','Ctrl 0']] as [label,key]}<div class="shortcut-row"><span>{label}</span><kbd>{key}</kbd></div>{/each}<p class="field-hint" style="margin-top:18px">Address and command shortcuts also work while a provider page has focus. Other shortcuts may be handled by the website.</p></Dialog>
