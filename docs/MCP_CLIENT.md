...
**Allow this tool for the rest of this task** is an optional Advanced control. It permits subsequent calls to the same tool **with different arguments** during that one task. It is deliberately labelled as broader than an exact-file/path grant. It is not on by default. It expires after 30 minutes, is revoked at a work-budget pause, and does not persist across task completion, cancellation, failure, reconnect or restart. Expiring a broad grant does not itself destroy the task; the next call requires consent.
...
Eight configured servers, 64 discovered tools per server, eight catalog pages, 32 argv values, 128 KiB stdio frames, 8 KiB tool arguments, 20 KiB tool results, one active built-in task and one tool call per model turn. At most 32 tools may be enabled for a task, with a 48 KiB combined tool-definition budget. Per-server admitted catalog bytes are bounded conservatively, including JSON escaping; counts do not override byte caps. An enable operation that exceeds the definition budget is rolled back.

Default work budget: 100 model turns and 30 active minutes. Trusted UI controls allow 10–500 turns / 5–240 minutes. Approval and manual-result waiting are excluded. Active generation/tool execution is allowed to settle before pausing at a safe boundary. A budget pause keeps the same volatile run/history/session and completed result; it clears broad grants. Only the same run ID, unchanged catalog/context and a WORK_BUDGET_REACHED pause can resume. Resume extends the budget; it does not restart a completed tool. This is not persistence across a crash or an MCP durable task handle.

Process setup and initialization receive 120 seconds. Catalog/discovery requests retain a 30-second bound. A tool call receives five minutes of inactivity tolerance; only its matching progress token refreshes that deadline. A separate one-hour absolute ceiling is not extended by progress. Stdio write timeout remains three seconds, with bounded queue and output-rate limits. Stderr is drained/discarded rather than exposed as possible secrets. Stop uses an independent cancellation notification, closes input and terminates remaining process-group descendants with a bounded grace period. None of this protects against malicious group escape or kernel stalls.

The host permits up to 1 MiB only for its internal mcp.transport envelope so JSON-escaped catalog frames can reach Zag. Ordinary UI/provider commands retain their 64 KiB limit and cannot invoke mcp.transport. The backend owns catalog admission and task state. Schema digests are calculated at admission rather than repeatedly hashing every raw schema during the task loop.

Transcripts, generation output, effective website context and protocol framing have independent finite limits. A task can still stop on provider auth/quota/mapping changes, context or payload exhaustion, incomplete output, a process failure or unsupported protocol result. No invisible new conversation, quota workaround or automatic replay of uncertain side effects is used.
...
This builds/runs native ordinary, file-permission, discovery/MCP and safe-continuation unit programs, then the native gateway and `tests/native_mcp.py` integration suites. `native_mcp.py` requires the actual Zag ELF. It launches a real independent Node stdio test server, reads a synthetic file only after native authorization, returns its unpredictable bytes through the same native session, and checks revocation after disabling the model. Browser events and Rust transport are test doubles in that suite; it is not a live website or Tauri test.
...
TOOL_LIBRARY.md records the five explicit setup recipes. They are not preinstalled servers or sandbox guarantees. Selecting a recipe changes only the draft. A main-webview-only native folder picker returns a path, not an access grant. Separate process review remains necessary, and every newly discovered tool starts disabled.
