...
`PACKAGE_METADATA.json` and `docs/LOCAL_COMMIT.txt` identify the local source commit. `RECOVERY.bundle`, package metadata and the manifest are delivery wrappers outside the commit; the manifest excludes MANIFEST.sha256 and PACKAGE_METADATA.json. The external receipt/checksum records the outer ZIP hash, size and member count. The bundle is verified with `git bundle verify` during packaging.
