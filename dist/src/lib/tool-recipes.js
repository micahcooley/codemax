...
export function recipeDefinition(id, folder = '', permissions = {}) {
    const recipe = toolRecipes.find(r => r.id === id);
    if (!recipe)
        throw Error('Unknown tool recipe.');
    ...
    if (recipe.bundled) {
        ...
        if (permissions.commands)
            args.push('--allow-trusted-commands');
    }
