/* app.svelte.ts generated by Svelte v5.57.1 */
...
class Application {
	...
	get errorProvider() {
		return $.get(this.#errorProvider);
	}

	set errorProvider(value) {
		$.set(this.#errorProvider, value, true);
	}
	...
	#loadingAt = $.state($.proxy({}));

	get loadingAt() {
		return $.get(this.#loadingAt);
	}

	set loadingAt(value) {
		$.set(this.#loadingAt, value, true);
	}
	...
	markLoading(id, on) {
		...
		if (on) {
			...
			this.loadingAt = { ...this.loadingAt, [id]: now };

			return;
		}

		const { [id]: _, ...rest } = this.loading;
		const { [id]: __, ...restAt } = this.loadingAt;

		this.loading = rest;
		this.loadingAt = restAt;

		let changed = false;
		const live = { ...this.loading };
		const at = { ...this.loadingAt };

		for (const key of Object.keys(live)) {
			const started = at[Number(key)] ?? 0;

			if (now - started > 30000) {
				delete live[Number(key)];
				delete at[Number(key)];
				changed = true;
			}
		}
	...
	#closedTabs = $.state($.proxy([]));

	get closedTabs() {
		return $.get(this.#closedTabs);
	}

	set closedTabs(value) {
		$.set(this.#closedTabs, value, true);
	}
