...
export default function Models($$anchor, $$props) {
	...
	button.__click = () => app.showPopup('add');
	...
	$.each(node_2, 17, () => app.providers, $.index, ($$anchor, p) => {
		var option_1 = root_1();
	...
	$.each(tbody, 21, () => $.get(visible), (model) => model.id, ($$anchor, model) => {
		...
		button_3.__click = () => app.openProvider($.get(model).provider.id);
		...
		button_4.__click = () => app.clipboard($.get(model).id);
		...
		button_5.__click = () => app.settings({ default_model: $.get(model).id });
		...
		$.template_effect(
			($0, $1, $2) => {
				...
				$.set_text(text_5, $.get(model).reasoning?.control_observed ? 'Control found' : 'Unknown');
				$.set_text(text_6, $.get(model).reasoning?.control_observed ? 'Observed on website' : 'Not asserted');
				$.set_text(text_7, $1);
