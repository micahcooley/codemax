...
import { initials, providerText, errorText } from '../lib/format.js';
...
var root_4 = $.from_html(`<section class="section"><h2>Local gateway</h2><div class="setting-row"><div><strong>Accept client connections</strong><p>Stop cancels active inference and closes the local HTTP listener. The desktop browser remains usable.</p></div><button class="toggle" role="switch" aria-label="Accept client connections"></button></div><div class="setting-row"><div><strong>Listening port</strong><p>Only 127.0.0.1 is allowed. Changing ports preserves existing requests until they finish.</p></div><form class="inline"><input type="number" aria-label="Gateway port" min="1024" max="65535" required=""/><button class="secondary">Apply</button></form></div><div class="setting-row"><div><strong>Local API key</strong><p>Regeneration revokes the old key and terminates connected clients. Website credentials are unaffected.</p></div><button class="secondary"><!>Regenerate</button></div><div class="setting-row"><div><strong>Default model alias</strong><p>Requests for <code>codemax/default</code> route to this explicitly selected model.</p></div><select aria-label="Default gateway model"><option>No default selected</option><!></select></div></section> <section class="section"><h2>Explicit fallback</h2><div class="setting-row"><div><strong>Fallback model</strong><p>Choose another available model for new sessions only. Existing conversations are never silently moved between websites.</p></div><select aria-label="Fallback model"><option>No fallback selected</option><!></select></div><div class="setting-row"><div><strong>Allow fallback before dispatch</strong><p>Only before a prompt is submitted, when the selected website reports a limit or is unavailable. No retry after generation begins.</p></div><button class="toggle" role="switch" aria-label="Allow fallback"></button></div><div class="note"><!><span>Fallback follows your explicit routing policy; it does not bypass a provider’s quota or create additional accounts.</span></div></section>`, 1);
var root_5 = $.from_html(`<option>No saved website profiles</option>`);
var root_6 = $.from_html(`<div class="setting-row"><div><strong>Model and reasoning options</strong><p>Exposure, context overrides and model preferences live in Provider settings.</p></div><button class="secondary">Provider settings</button></div>`);
var root_7 = $.from_html(`<div class="setting-row"><div><strong>Display name</strong><p> </p></div><input aria-label="Website display name" maxlength="120"/></div><div class="setting-row"><div><strong>Keep this website awake</strong><p>Kept-awake profiles are excluded from idle suspension.</p></div><button class="toggle" role="switch" aria-label="Keep website awake"></button></div><!><div class="setting-row"><div><strong>Clear local profile</strong><p>Delete this website’s local storage and sign out of this profile. Remote account data is not deleted.</p></div><button class="secondary">Clear storage</button></div><div class="setting-row"><div><strong>Remove website</strong><p>Remove the profile and connector from the workspace and end associated sessions.</p></div><button class="secondary danger-text">Remove</button></div>`, 1);
var root_8 = $.from_html(`<p>No website profiles have been added.</p>`);
var root_9 = $.from_html(`<section class="section"><div class="section-title"><h2>Website profiles</h2><button class="secondary"><!>Add website</button></div><label class="field"><span>Website</span><select aria-label="Website profile"><!><!></select></label><!></section>`);
var root_10 = $.from_html(`<section class="section"><h2>Privacy boundaries</h2><div class="setting-row"><div><strong>Provider authentication</strong><p>Passwords, cookies, OAuth tokens, and authentication headers remain in website-owned browser storage. Instrumentation does not extract them.</p></div><!></div><div class="setting-row"><div><strong>Conversation storage</strong><p>Session IDs, conversation URLs, and SHA-256 message receipts persist locally. Prompt bodies and tool results are not written to session files.</p></div><span class="tag">Metadata only</span></div><div class="setting-row"><div><strong>Temporary chats for automated work</strong><p>When a website offers a temporary or incognito chat switch, coding-client generations start new conversations with it on. Manual browsing is never changed, and existing conversations are never flipped mid-chat.</p></div><button class="toggle" role="switch" aria-label="Temporary chats for automated work"></button></div><div class="setting-row"><div><strong>Raw payload capture</strong><p>Raw network payload and credential capture are not enabled by any preference in this build.</p></div><span class="tag">Disabled</span></div></section><section class="section"><h2>Diagnostics</h2><div class="setting-row"><div><strong>Log level</strong><p>Debug adds bounded structural browser events. Authentication values and form data remain excluded.</p></div><select aria-label="Log level"></select></div><div class="setting-row"><div><strong>Developer diagnostics</strong><p>Enable authenticated debug endpoints and allow the native page inspector. This never grants arbitrary commands to provider pages.</p></div><button class="toggle" role="switch" aria-label="Developer diagnostics"></button></div><div class="setting-row"><div><strong>Retained diagnostic events</strong><p>The ring holds at most 160 redacted records and is cleared when the backend exits.</p></div><button class="secondary">Clear events</button></div></section>`, 1);
var root_11 = $.from_html(`<section class="section"><h2>Desktop runtime</h2><div class="setting-row"><div><strong> </strong><p> </p></div><span><span></span> </span></div><div class="setting-row"><div><strong>Backend version</strong><p>Protocol version 1 · Linux x86-64 distribution target · fallback is macOS browsing-only, not release-qualified</p></div><code> </code></div><div class="setting-row"><div><strong>Restart backend</strong><p> </p></div><button class="secondary"><!> </button></div><div class="setting-row"><div><strong>Keyboard controls</strong><p>Review the desktop shortcuts and website-focus behavior.</p></div><button class="secondary">View shortcuts</button></div></section><section class="section"><h2>Research detector</h2><p style="margin-top:15px"> </p><div class="note"><!><span>TNN action proposals must pass the same bounded browser action validator. An unqualified detector cannot take over production behavior.</span></div></section>`, 1);
var root_12 = $.from_html(`<section class="screen"><div class="screen-inner"><header class="screen-head"><div><div class="breadcrumb">Workspace / Preferences</div><h1>Settings</h1><p>Changes are saved by the Zag backend. Browser permissions remain explicit.</p></div></header><div class="preferences-layout"><nav class="preferences-nav" aria-label="Settings sections"></nav><div><!></div></div></div></section>`);

export default function Settings($$anchor, $$props) {
	$.push($$props, true);

	const tab = $.derived(() => app.settingsSection);
	let port = $.state('');
	...
	async function restart() {
		if (app.sessions.some((s) => s.status === 'ACTIVE') || app.activeTask) {
			await app.ask('Restart and interrupt active work?', 'Active client requests and website tool tasks will be interrupted. Saved profiles will be reloaded.', 'Restart backend', async () => await doRestart() ? true : undefined);
		} else await doRestart();
	}

	async function applyPort() {
		if (await app.settings({ port: Number($.get(port)) })) $.set(port, '');
	}
	...
	$.each(nav, 21, () => sections, $.index, ($$anchor, section) => {
		...
		$.template_effect(() => {
			$.set_attribute(button, 'aria-current', $.get(tab) === $.get(section).id ? 'page' : undefined);
			classes = $.set_class(button, 1, '', null, classes, { active: $.get(tab) === $.get(section).id });
			$.set_text(text, $.get(section).label);
		});

		$.delegated('click', button, () => app.settingsSection = $.get(section).id);
...

	{
		var consequent = ($$anchor) => {
			...
			$.each(div_4, 20, () => ['dark', 'light', 'system'], $.index, ($$anchor, theme) => {
				...
				$.template_effect(
					($0) => {
						$.set_attribute(button_1, 'aria-pressed', app.preferences.theme === theme);
				...
				$.delegated('click', button_1, () => app.settings({ theme }));
			...
			$.delegated('click', button_2, () => app.settings({ compact: !app.preferences.compact }));
			...
			$.delegated('click', button_4, () => app.settings({ restore_tabs: !app.preferences.restore_tabs }));
			$.delegated('click', button_5, () => app.settings({ auto_start: !app.preferences.auto_start }));
			$.delegated('change', select, (event) => app.settings({ idle_minutes: Number(event.currentTarget.value) }));
		...
		var consequent_1 = ($$anchor) => {
			...
			var node_2 = $.child(button_8);

			Icon(node_2, { name: 'key', size: 14 });
			$.next();
			$.reset(button_8);
			$.reset(div_12);
			...
			$.each(node_3, 17, () => app.models, $.index, ($$anchor, m) => {
				var option_7 = root_3();
				var text_2 = $.only_child(option_7);
				var option_7_value = {};

				$.template_effect(() => {
					option_7.disabled = !$.get(m).enabled || !$.get(m).available || !$.get(m).provider.exposed || $.get(m).provider.state !== 'READY';
			...
			$.each(node_4, 17, () => app.models, $.index, ($$anchor, m) => {
				var option_9 = root_3();
				var text_3 = $.only_child(option_9);
				var option_9_value = {};

				$.template_effect(() => {
					option_9.disabled = !$.get(m).enabled || !$.get(m).available || !$.get(m).provider.exposed || $.get(m).provider.state !== 'READY';
			...
			Icon(node_5, { name: 'shield', size: 15 });
			$.next();
			$.reset(div_16);
			$.reset(section_5);
			...
			$.template_effect(
				...
				[
					() => !app.ready || !$.get(port) || Number($.get(port)) === app.snapshot?.api.port || app.busy('settings.update')
				]
			);

			$.delegated('click', button_6, () => app.snapshot?.api.running
				? app.ask('Stop the local gateway?', 'This cancels connected client requests. Website tabs and login data are kept.', 'Stop gateway', () => app.perform('api.stop'))
				: app.perform('api.start'));
			...
			$.delegated('input', input, (event) => $.set(port, event.currentTarget.value, true));
			$.delegated('click', button_8, () => app.showPopup('rotate-key'));
			$.delegated('change', select_1, (event) => app.settings({ default_model: event.currentTarget.value }));

			$.delegated('change', select_2, (event) => app.settings({
				fallback_model: event.currentTarget.value,
				fallback_enabled: !!event.currentTarget.value && app.preferences.fallback_enabled
			}));

			$.delegated('click', button_9, () => app.settings({ fallback_enabled: !app.preferences.fallback_enabled }));
		...
		var consequent_5 = ($$anchor) => {
			...
			var node_6 = $.child(button_10);

			Icon(node_6, { name: 'plus', size: 13 });
			$.next();
			$.reset(button_10);
			$.reset(div_17);
			...
			var node_7 = $.child(select_3);

			{
				var consequent_2 = ($$anchor) => {
					var option_10 = root_5();
				...
				$.if(node_7, ($$render) => {
					if (!app.providers.length) $$render(consequent_2);
				});
			}

			var node_8 = $.sibling(node_7);

			$.each(node_8, 17, () => app.providers, $.index, ($$anchor, p) => {
				var option_11 = root_3();
				var text_4 = $.only_child(option_11);
				var option_11_value = {};
			...
			var node_9 = $.sibling(label);

			{
				var consequent_4 = ($$anchor) => {
					var fragment_2 = root_7();
					var div_18 = $.first_child(fragment_2);
...

					{
						var consequent_3 = ($$anchor) => {
							...
							$.delegated('click', button_12, () => {
								app.providerSelection = $.get(profile).id;
						...
						$.if(node_10, ($$render) => {
							if ($.get(profile).detected && !$.get(profile).dismissed) $$render(consequent_3);
						});
					}

					var div_22 = $.sibling(node_10);
					var button_13 = $.sibling($.child(div_22));

					$.reset(div_22);

					var div_23 = $.sibling(div_22);
					var button_14 = $.sibling($.child(div_23));
					...
					$.delegated('change', input_1, (event) => updateProfile('label', event.currentTarget.value));
					$.delegated('click', button_11, () => updateProfile('pinned', !$.get(profile).pinned));
					$.delegated('click', button_13, () => app.showPopup('clear-profile', $.get(profile).id));
					$.delegated('click', button_14, () => app.showPopup('remove-provider', $.get(profile).id));
				...
				$.if(node_9, ($$render) => {
					if ($.get(profile)) $$render(consequent_4); else $$render(alternate, -1);
				});
			}
			...
			$.template_effect(() => {
				...
				if (select_3_value !== (select_3_value = $.get(profile)?.id ?? '')) {
					(
						select_3.value = (select_3.__value = select_3_value) ?? '',
						$.select_option(select_3, select_3_value)
					);
				}
			});

			$.delegated('click', button_10, () => app.showPopup('add'));
			$.delegated('change', select_3, (event) => $.set(selectedProfile, event.currentTarget.value, true));
			$.append($$anchor, section_6);
		};

		var consequent_6 = ($$anchor) => {
			var fragment_3 = root_10();
			var section_7 = $.first_child(fragment_3);
			var div_24 = $.sibling($.child(section_7));
			var node_11 = $.sibling($.child(div_24));

			Icon(node_11, { name: 'lock', size: 17 });
			$.reset(div_24);

			var div_25 = $.sibling(div_24, 2);
			var button_15 = $.sibling($.child(div_25));
			...
			$.each(select_4, 20, () => ['ERROR', 'WARN', 'INFO', 'DEBUG'], $.index, ($$anchor, level) => {
				var option_12 = root_3();
				var text_6 = $.only_child(option_12, true);
				var option_12_value = {};
			...
			var button_16 = $.sibling($.child(div_27));

			$.reset(div_27);

			var div_28 = $.sibling(div_27);
			var button_17 = $.sibling($.child(div_28));

			$.reset(div_28);
			$.reset(section_8);

			$.template_effect(() => {
				$.set_attribute(button_15, 'aria-checked', app.preferences.ephemeral_chats);
				button_15.disabled = !app.ready;

				if (select_4_value !== (select_4_value = app.preferences.logging)) {
					(
						select_4.value = (select_4.__value = select_4_value) ?? '',
						$.select_option(select_4, select_4_value)
					);
				}

				$.set_attribute(button_16, 'aria-checked', app.preferences.developer_mode);
				button_16.disabled = !app.ready;
				button_17.disabled = !app.events.length;
			});

			$.delegated('click', button_15, () => app.settings({ ephemeral_chats: !app.preferences.ephemeral_chats }));
			$.delegated('change', select_4, (event) => app.settings({ logging: event.currentTarget.value }));
			$.delegated('click', button_16, () => app.settings({ developer_mode: !app.preferences.developer_mode }));
			$.delegated('click', button_17, () => app.perform('logs.clear'));
			$.append($$anchor, fragment_3);
		};

		var alternate_1 = ($$anchor) => {
			var fragment_4 = root_11();
			var section_9 = $.first_child(fragment_4);
			var div_29 = $.sibling($.child(section_9));
			var div_30 = $.child(div_29);
			var strong = $.child(div_30);
			var text_7 = $.only_child(strong, true);
			var p_3 = $.sibling(strong);
			...
			let classes_3;
			var text_9 = $.sibling(span_1, 1, true);
			...
			var p_4 = $.sibling($.child(div_33));
			var text_11 = $.only_child(p_4, true);

			$.reset(div_33);

			var button_18 = $.sibling(div_33);
			var node_12 = $.child(button_18);

			Icon(node_12, { name: 'refresh', size: 14 });

			var text_12 = $.sibling(node_12, 1, true);

			$.reset(button_18);
			$.reset(div_32);

			var div_34 = $.sibling(div_32);
			var button_19 = $.sibling($.child(div_34));

			$.reset(div_34);
			$.reset(section_9);

			var section_10 = $.sibling(section_9);
			var p_5 = $.sibling($.child(section_10));
			var text_13 = $.only_child(p_5, true);
			var div_35 = $.sibling(p_5);
			var node_13 = $.child(div_35);
			...
			$.template_effect(
				($0) => {
					$.set_text(text_7, app.host.code === 'FALLBACK_MODE' ? 'Browsing-only mode' : 'Zag sidecar');
					$.set_text(text_8, $0);
					...
					$.set_text(text_9, app.host.state);
					$.set_text(text_10, app.snapshot?.version || 'Not connected');

					$.set_text(text_11, app.host.code === 'FALLBACK_MODE'
						? 'Retry the Zag sidecar now. Browsing profiles stay on disk while Codemax retries automatically in the background.'
						: 'Cancel current requests, restart the sidecar, reload saved metadata, and reconnect the trusted UI. Provider profiles remain on disk.');

					button_18.disabled = $.get(restarting);

					$.set_text(text_12, $.get(restarting)
						? 'Restarting…'
						: app.host.code === 'FALLBACK_MODE' ? 'Retry Zag now' : 'Restart Zag');

					$.set_text(text_13, app.snapshot?.detector.reason || 'No qualified TNN artifact is loaded. Symbolic discovery remains authoritative.');
				},
				[
					() => app.host.code
						? errorText(app.host.code)
						: 'The native backend owns routing, protocol translation, sessions, discovery, and the loopback server.'
				]
			);

			$.delegated('click', button_18, restart);
			$.delegated('click', button_19, () => app.showPopup('shortcuts'));
			$.append($$anchor, fragment_4);
		};

		$.if(node, ($$render) => {
			if ($.get(tab) === 'appearance') $$render(consequent); else if ($.get(tab) === 'gateway') $$render(consequent_1, 1); else if ($.get(tab) === 'profiles') $$render(consequent_5, 2); else if ($.get(tab) === 'privacy') $$render(consequent_6, 3); else $$render(alternate_1, -1);
		});
	}
