...
export default function Harness($$anchor, $$props) {
	...
	const boundModel = $.derived(() => app.models.find((m) => m.id === $.get(probe)?.model));
	...
	var section_1 = $.sibling(div_3, 2);
	var label = $.child(section_1);
	...
	$.each(node_2, 17, () => app.models, (m) => m.id, ($$anchor, m) => {
		var option_1 = root_1();
	...
	var node_7 = $.sibling($.child(section_2), 6);
		...
		var consequent_7 = ($$anchor) => {
...

			{
				var consequent_2 = ($$anchor) => {
					...
					var div_8 = $.sibling($.first_child(fragment_1));
				...
				var alternate_3 = ($$anchor) => {
					...
					$.template_effect(() => {
						classes = $.set_class(div_11, 1, 'proof-step', null, classes, { done: $.get(probe).read_count === 1 });
						classes_1 = $.set_class(div_12, 1, 'proof-step', null, classes_1, { done: $.get(probe).proof_verified });
					});
	...
	var section_3 = $.child(aside);
	var div_14 = $.child(section_3);
	var node_20 = $.sibling($.child(div_14));

	Icon(node_20, { name: 'shield', size: 15 });
	$.reset(div_14);

	var table = $.sibling(div_14);
	...
	var node_21 = $.sibling($.child(details), 2);

	ExternalClients(node_21, {});
	$.reset(details);
	$.reset(div);
	$.reset(section);
	...
	$.template_effect(() => {
		...
		select.disabled = !app.models.length || $.get(busy) || $.get(needsPermission);
