...
var root_2 = $.from_html(`<p class="error-banner" role="alert"> </p>`);
var root_3 = $.from_html(`<div class="tool-call"><div class="section-title"><h3>Approve this tool call</h3><!></div><p><strong> </strong> </p><pre class="code"> </pre><p class="field-hint"> </p> <details class="advanced-client"><summary>Advanced · automatic execution for this task</summary><label class="setting-line"><div><strong>Allow this tool for the rest of this task</strong><p>Includes future calls with different arguments. This is not a path-limited file permission. The grant ends with this task or a connection change.</p></div><input type="checkbox" aria-label="Allow MCP tool for task"/></label></details> <div class="button-group"><button class="primary"> </button><button class="secondary">Deny and stop</button></div></div>`);
var root_4 = $.from_html(`<p>The approved tool finished. Send its result to continue the website conversation.</p><button class="primary">Insert result and send</button>`, 1);
var root_5 = $.from_html(`<pre class="tool-answer"> </pre>`);
var root_6 = $.from_html(`<details class="advanced-client"><summary>Last tool result · untrusted data</summary><pre class="code"> </pre></details>`);
var root_7 = $.from_html(`<div class="button-group" style="margin-top:15px"><button class="secondary danger-text">Stop task and revoke permissions</button><button class="text-button">Show website</button></div>`);
var root_1 = $.from_html(`<section class="section task-activity" aria-label="MCP task activity"><div class="section-title"><h2>Task activity</h2><span> </span></div> <!> <!> <!> <!> <!> <!></section>`);
var root_9 = $.from_html(`<option>Select an available model</option>`);
var root_10 = $.from_html(`<option> </option>`);
var root_11 = $.from_html(`<button class="text-button">Show website</button>`);
var root_8 = $.from_html(`<section class="section"><div class="section-title"><h2>Start a task</h2><span class="tag"> </span></div> <label class="field"><span>Website model</span><select aria-label="MCP website model"><!><!></select></label> <label class="field"><span>What should the model do?</span><textarea aria-label="MCP task" maxlength="8191" rows="3" placeholder="Describe a task for the selected website model"></textarea><span class="field-hint">Your task, enabled tool descriptions and approved results are shared with the selected website. Drafts stay in memory until you close the application.</span></label> <label class="setting-line"><div><strong>Automatically continue after approved tools</strong><p>Insert results and send the next turn. Tool execution still requires your approval.</p></div><input type="checkbox" aria-label="Automatically continue tool results"/></label> <div class="button-group"><button class="primary"><!> </button><!></div> <p class="field-hint" role="status" style="margin-top:12px"> </p></section>`);
var root_14 = $.from_html(`<option> </option>`);
var root_13 = $.from_html(`<label class="field"><span>Imported definition</span><select></select><span class="field-hint">Command and arguments only. No credentials or environment values are copied.</span></label>`);
var root_15 = $.from_html(`<p class="form-error" role="alert"> </p>`);
var root_12 = $.from_html(`<section class="section" aria-label="MCP server editor"><h2>Add an MCP server</h2><form><!> <label class="field"><span>Name</span><input aria-label="MCP server name" maxlength="120" required/></label> <label class="field"><span>Executable</span><input class="mono" aria-label="MCP executable" maxlength="2000" required/><span class="field-hint">An installed command or absolute executable path. Arguments go below; do not paste a shell command.</span></label> <label class="field"><span>Arguments · JSON array</span><textarea class="mono" aria-label="MCP arguments" rows="3"></textarea></label> <!> <div class="button-group"><button class="primary"> </button><button type="button" class="secondary">Cancel</button></div></form></section>`);
var root_16 = $.from_html(`<section tabindex="-1" class="section permission-box" aria-label="Local process permission"><div class="section-title"><h2>Allow this local process?</h2><!></div><p><strong> </strong> runs with your computer user account’s access. A package runner may download code. Tool approval does not sandbox the server itself.</p><pre class="code"> </pre><label class="setting-line"><span>I trust this executable and its arguments.</span><input type="checkbox" aria-label="Trust MCP executable"/></label><div class="button-group"><button class="primary"> </button><button class="secondary">Do not run</button></div></section>`);
var root_17 = $.from_html(`<div class="discovery-note"><!><div>No tool servers configured. Import an existing MCP configuration or add a trusted executable. Your coding client’s model connection does not require an MCP server.</div></div>`);
var root_19 = $.from_html(`<button class="secondary">Connect</button>`);
var root_20 = $.from_html(`<button class="secondary">Disconnect</button>`);
var root_21 = $.from_html(`<p class="error-banner" role="alert"> </p>`);
var root_22 = $.from_html(`<label class="setting-line"><div><strong> </strong><p> </p></div><input type="checkbox"/></label>`);
var root_23 = $.from_html(`<details><summary> </summary><pre class="code"> </pre></details>`);
var root_18 = $.from_html(`<div class="tool-server"><div class="section-title"><div><strong> </strong><p class="field-hint"> </p></div><div class="button-group"><!></div></div> <!> <!> <details class="advanced-client"><summary>Advanced · executable and tool schemas</summary><p class="field-hint"> </p><pre class="code"> </pre><!><p class="field-hint">Descriptions are supplied by the server, not a security guarantee. Reconnecting or a changed catalog resets enabled choices and task grants.</p><button class="text-button danger-text">Remove server</button></details></div>`);
var root = $.from_html(`<section class="screen"><div class="screen-inner"><header class="screen-head"><div><div class="breadcrumb">Codemax / Website tasks</div><h1>Tools & MCP</h1><p>Give a website model access to tools you choose. You approve execution; Codemax returns results to the same conversation.</p></div><div class="button-group"><button class="secondary">Import configuration</button><button class="secondary"><!>Add tool server</button></div></header> <!> <!> <!> <!> <section class="section"><div class="section-title"><h2>Tool servers</h2><span class="tag"> </span></div> <!> <!></section> <details class="advanced-client"><summary>Advanced · synthetic file permission diagnostic</summary><!></details></div></section>`);
...
export default function Tools($$anchor, $$props) {
	...
	const blocker = $.derived(() => !app.ready
		...
		: !$.get(model)
			...
			: !$.get(enabled).length
				...
				: !$.get(draft).task.trim() ? 'Describe what the model should do.' : '');
	...
	button.__click = importServers;

	var button_1 = $.sibling(button);

	button_1.__click = () => $.get(draft).adding = !$.get(draft).adding;
...

	{
		var consequent_6 = ($$anchor) => {
...

			{
				...
				$.if(node_2, ($$render) => {
					if ($.get(run).error && $.get(run).state !== 'CANCELLED') $$render(consequent);
				});
			}
...

			{
				var consequent_1 = ($$anchor) => {
					...
					var node_4 = $.sibling($.child(div_4));

					Icon(node_4, { name: 'shield', size: 17 });
					$.reset(div_4);

					var p_2 = $.sibling(div_4);
					...
					var details = $.sibling(p_3, 2);
					var label = $.sibling($.child(details));
					...
					$.template_effect(
						($0, $1, $2, $3, $4) => {
							$.set_text(text_2, $.get(pending)?.name || $.get(run).tool);
							$.set_text(text_3, ` · ${($.get(pending)?.server.label || 'Server unavailable') ?? ''}`);
							$.set_text(text_4, $.get(run).arguments);
							$.set_text(text_5, `The result will be sent to ${$0 ?? ''}. Check paths, commands and destinations before approving.`);
							input.disabled = $1;
							button_2.disabled = $2;
							$.set_text(text_6, $3);
							button_3.disabled = $4;
				...
				$.if(node_3, ($$render) => {
					if ($.get(run).state === 'PERMISSION_REQUIRED') $$render(consequent_1);
				});
			}
...

			{
				var consequent_2 = ($$anchor) => {
					var fragment = root_4();
					var button_4 = $.sibling($.first_child(fragment));

					button_4.__click = () => app.perform('mcp.run.continue', { run_id: $.get(run).id });
					$.template_effect(($0) => button_4.disabled = $0, [() => !app.ready || app.busy('mcp.run.continue')]);
					$.append($$anchor, fragment);
				};

				$.if(node_5, ($$render) => {
					if ($.get(run).state === 'RESULT_READY') $$render(consequent_2);
				});
			}
...

			{
				var consequent_3 = ($$anchor) => {
					...
					$.reset(pre_1);
					$.template_effect(() => $.set_text(text_7, $.get(run).answer));
					$.append($$anchor, pre_1);
				};

				$.if(node_6, ($$render) => {
					if ($.get(run).answer) $$render(consequent_3);
				});
			}
...

			{
				var consequent_4 = ($$anchor) => {
					...
					var pre_2 = $.sibling($.child(details_1));
					var text_8 = $.child(pre_2, true);

					$.reset(pre_2);
					$.reset(details_1);
					$.template_effect(() => $.set_text(text_8, $.get(run).last_result));
					$.append($$anchor, details_1);
				};

				$.if(node_7, ($$render) => {
					if ($.get(run).last_result) $$render(consequent_4);
				});
			}
...

			{
				var consequent_5 = ($$anchor) => {
					...
					button_6.__click = () => app.openProvider($.get(run).provider_id);
					$.reset(div_6);
				...
				$.if(node_8, ($$render) => {
					if ($.get(running)) $$render(consequent_5);
				});
			}
		...
		$.if(node_1, ($$render) => {
			if ($.get(run) && $.get(run).state !== 'IDLE') $$render(consequent_6);
		});
	}
...

	{
		var consequent_9 = ($$anchor) => {
			...
			$.each(node_11, 17, () => app.exposedModels, (item) => item.id, ($$anchor, item) => {
				...
				$.template_effect(() => {
					$.set_text(text_10, `${$.get(item).display_name ?? ''} · ${$.get(item).provider.label ?? ''}`);
			...
			var div_8 = $.sibling(label_3, 2);
			var button_7 = $.child(div_8);

			button_7.__click = start;

			var node_12 = $.child(button_7);

			Icon(node_12, { name: 'arrow', size: 15 });

			var text_11 = $.sibling(node_12, 1, true);

			$.reset(button_7);

			var node_13 = $.sibling(button_7);

			{
				var consequent_8 = ($$anchor) => {
					var button_8 = root_11();

					button_8.__click = () => app.openProvider($.get(model).provider.id);
					$.template_effect(() => button_8.disabled = !app.ready);
					$.append($$anchor, button_8);
				};

				$.if(node_13, ($$render) => {
					if ($.get(model)) $$render(consequent_8);
				});
			}
			...
			$.template_effect(
				($0, $1) => {
					$.set_text(text_9, `${$.get(enabled).length ?? ''} enabled ${$.get(enabled).length === 1 ? 'tool' : 'tools'}`);
					...
					button_7.disabled = $0;
					$.set_text(text_11, $1);
					$.set_text(text_12, $.get(blocker) || 'Starts a new website conversation. Up to 20 model turns and 10 minutes per task.');
				},
				[
		...
		$.if(node_9, ($$render) => {
			if (!$.get(running)) $$render(consequent_9);
		});
	}
...

	{
		var consequent_12 = ($$anchor) => {
			var section_3 = root_12();
			var form = $.sibling($.child(section_3));
			var node_15 = $.child(form);

			{
				var consequent_10 = ($$anchor) => {
					...
					$.each(select_1, 21, () => $.get(imported), $.index, ($$anchor, definition, i) => {
						...
						$.template_effect(() => $.set_text(text_13, $.get(definition).label));
				...
				$.if(node_15, ($$render) => {
					if ($.get(imported).length) $$render(consequent_10);
				});
			}

			var label_5 = $.sibling(node_15, 2);
...

			{
				var consequent_11 = ($$anchor) => {
					...
					$.reset(p_5);
					$.template_effect(() => $.set_text(text_14, $.get(formError)));
					$.append($$anchor, p_5);
				};

				$.if(node_16, ($$render) => {
					if ($.get(formError)) $$render(consequent_11);
				});
			}
			...
			button_10.__click = () => $.get(draft).adding = false;
			$.reset(div_9);
			$.reset(form);
			$.reset(section_3);
			...
			$.bind_value(input_2, () => $.get(draft).label, ($$value) => $.get(draft).label = $$value);
			$.bind_value(input_3, () => $.get(draft).command, ($$value) => $.get(draft).command = $$value);
			$.bind_value(textarea_1, () => $.get(draft).argv, ($$value) => $.get(draft).argv = $$value);
			$.append($$anchor, section_3);
		};

		$.if(node_14, ($$render) => {
			if ($.get(draft).adding) $$render(consequent_12);
		});
	}
...

	{
		var consequent_13 = ($$anchor) => {
			...
			var div_10 = $.child(section_4);
			var node_18 = $.sibling($.child(div_10));

			Icon(node_18, { name: 'shield', size: 18 });
			$.reset(div_10);

			var p_6 = $.sibling(div_10);
			var strong_1 = $.child(p_6);
			var text_16 = $.child(strong_1, true);
			...
			var text_17 = $.child(pre_3);

			$.reset(pre_3);

			var label_8 = $.sibling(pre_3);
			var input_4 = $.sibling($.child(label_8));

			$.remove_input_defaults(input_4);
			$.reset(label_8);

			var div_11 = $.sibling(label_8);
			var button_11 = $.child(div_11);

			button_11.__click = connect;

			var text_18 = $.child(button_11, true);
			...
			$.reset(div_11);
			$.reset(section_4);
			$.bind_this(section_4, ($$value) => $.set(consentPanel, $$value), () => $.get(consentPanel));

			$.template_effect(
				($0, $1, $2) => {
					$.set_text(text_16, $.get(consent).label);

					$.set_text(text_17, `${$.get(consent).command ?? ''}
${$0 ?? ''}`);

					button_11.disabled = $1;
					$.set_text(text_18, $2);
				},
				[
			...
			$.bind_checked(input_4, () => $.get(trust), ($$value) => $.set(trust, $$value));
			$.append($$anchor, section_4);
		};

		$.if(node_17, ($$render) => {
			if ($.get(consent)) $$render(consequent_13);
		});
	}

	var section_5 = $.sibling(node_17, 2);
	var div_12 = $.child(section_5);
	var span_2 = $.sibling($.child(div_12));
	var text_19 = $.child(span_2);
...

	{
		var consequent_14 = ($$anchor) => {
			...
			Icon(node_20, { name: 'terminal', size: 16 });
			$.next();
			$.reset(div_13);
		...
		$.if(node_19, ($$render) => {
			if (!$.get(servers).length) $$render(consequent_14);
		});
	}

	var node_21 = $.sibling(node_19, 2);

	$.each(node_21, 17, () => $.get(servers), (server) => server.id, ($$anchor, server) => {
		var div_14 = root_18();
		var div_15 = $.child(div_14);
		var div_16 = $.child(div_15);
		var strong_2 = $.child(div_16);
		var text_20 = $.child(strong_2, true);

		$.reset(strong_2);

		var p_7 = $.sibling(strong_2);
		var text_21 = $.child(p_7);
...

		{
			var consequent_15 = ($$anchor) => {
				...
				$.template_effect(() => button_13.disabled = $.get(running) || !app.ready);
				$.append($$anchor, button_13);
			};

			var alternate = ($$anchor) => {
				var button_14 = root_20();

				button_14.__click = () => app.perform('mcp.server.disconnect', { server_id: $.get(server).id });

				$.template_effect(($0) => button_14.disabled = $0, [
			...
			$.if(node_22, ($$render) => {
				if (['DISCONNECTED', 'ERROR'].includes($.get(server).state)) $$render(consequent_15); else $$render(alternate, false);
			});
		}
...

		{
			var consequent_16 = ($$anchor) => {
				var p_8 = root_21();
				var text_22 = $.child(p_8, true);

				$.reset(p_8);
				$.template_effect(($0) => $.set_text(text_22, $0), [() => errorText($.get(server).error)]);
				$.append($$anchor, p_8);
			};

			$.if(node_23, ($$render) => {
				if ($.get(server).error) $$render(consequent_16);
			});
		}

		var node_24 = $.sibling(node_23, 2);

		$.each(node_24, 17, () => $.get(server).tools, (tool) => tool.alias, ($$anchor, tool) => {
			var label_9 = root_22();
			var div_18 = $.child(label_9);
			var strong_3 = $.child(div_18);
			var text_23 = $.child(strong_3, true);
			...
			$.template_effect(() => {
				$.set_text(text_23, $.get(tool).name);
				$.set_text(text_24, $.get(tool).description);
				$.set_attribute(input_5, 'aria-label', `Enable MCP tool ${$.get(tool).name}`);
				$.set_checked(input_5, $.get(tool).enabled);
				input_5.disabled = $.get(running) || !app.ready || $.get(server).state !== 'READY';
			});

			$.append($$anchor, label_9);
		});

		var details_2 = $.sibling(node_24, 2);
		var p_10 = $.sibling($.child(details_2));
		var text_25 = $.child(p_10);

		$.reset(p_10);

		var pre_4 = $.sibling(p_10);
		var text_26 = $.child(pre_4);

		$.reset(pre_4);

		var node_25 = $.sibling(pre_4);

		$.each(node_25, 17, () => $.get(server).tools, (tool) => tool.alias, ($$anchor, tool) => {
			var details_3 = root_23();
			var summary = $.child(details_3);
			var text_27 = $.child(summary, true);

			$.reset(summary);

			var pre_5 = $.sibling(summary);
			var text_28 = $.child(pre_5, true);
			...
			$.template_effect(
				($0) => {
					$.set_text(text_27, $.get(tool).name);
					$.set_text(text_28, $0);
				},
		...
		var button_15 = $.sibling(node_25, 2);

		button_15.__click = () => remove($.get(server));
		$.reset(details_2);
		$.reset(div_14);

		$.template_effect(
			($0, $1, $2, $3) => {
				$.set_text(text_20, $.get(server).label);
				$.set_text(text_21, `${$0 ?? ''} · ${$1 ?? ''} ${$2 ?? ''} enabled`);
				$.set_text(text_25, `MCP ${($.get(server).protocol || 'not negotiated') ?? ''} · local stdio`);

				$.set_text(text_26, `${$.get(server).command ?? ''}
${$3 ?? ''}`);

				button_15.disabled = $.get(running) || !app.ready;
			},
			[
	...
	var details_4 = $.sibling(section_5, 2);
	var node_26 = $.sibling($.child(details_4));
	...
	$.template_effect(() => {
		button.disabled = $.get(running) || !app.ready;
		button_1.disabled = $.get(running) || !app.ready;
		$.set_attribute(button_1, 'aria-expanded', $.get(draft).adding);
		$.set_text(text_19, `${$.get(servers).length ?? ''} configured`);
	});
